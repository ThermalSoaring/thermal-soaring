...
def batchProcessing(df, slidingWindow=30, slideBy=15,
    ...
    for i in range(0, int(slidingWindows)):
        ...
        # Run GPR
        x, y, prediction, uncertainty = ThermalGPRPlot(fig, timepos, measurements,
                GPRParams(theta0=1e-2, thetaL=1e-10, thetaU=1e10, nugget=1,
                    random_start=10), extent=0, points=200)
...
if __name__ == "__main__":
    # Let's do this thing
    df = pd.read_csv('../bayesian-learning/gaussian-process-regression/run9.csv')
